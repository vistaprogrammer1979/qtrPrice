...
@Singleton
public class CusDentistryPriceListItemClient extends ClientBase {
    @Inject
    Logger logger;
    @Inject
        ...
        if (activities != null) {
            for (var activity : activities) {
                for (var cusContract : cusContracts) {
                    if (cusContract.getDentistryAsdsgPriceListId() != null) {
                        lst.add(String.format("%s:%s:%s", "CusDentistryASDSGPriceListItem", cusContract.getDentistryAsdsgPriceListId(), activity.getCode()));
                    }
                }
            }
        }
    ...
    @Override
    @WithSpan("Add Facts For CusPriceListItem_Dentistry_ASDSG")
    public Uni<Void> addFacts(Data data) {
        data.setCusDentistryAsdsgPriceListItems(new ArrayList<>());
        ...
        return redisService.getData(this.getKeys(data)).onItem().transformToUni(fetchedData -> Uni.createFrom().voidItem().invoke(() -> {
            for (var map : fetchedData) {
                if (!map.isEmpty()){
                    var obj = objectMapper.convertValue(map, CusDentistryASDSGPriceListItem.class);
                    data.getCusDentistryAsdsgPriceListItems().add(obj);
                }
            }
        }));
