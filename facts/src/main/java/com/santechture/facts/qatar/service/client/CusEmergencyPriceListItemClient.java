...
@Singleton
public class CusEmergencyPriceListItemClient extends ClientBase {
    @Inject
    Logger logger;
    @Inject
        ...
        if (activities != null) {
            for (var activity : activities) {
                for (var cusContract : cusContracts) {
                    if (cusContract.getEmergencyUrgPriceListId() != null) {
                        lst.add(String.format("%s:%s:%s", "CusEmergencyURGPriceListItem", cusContract.getEmergencyUrgPriceListId(), activity.getCode()));
                    }
                }
            }
        }
    ...
    @Override
    @WithSpan("Add Facts For CusPriceListItemEmergencyUrgsList")
    public Uni<Void> addFacts(Data data) {
        data.setCusEmergencyUrgPriceListItems(new ArrayList<>());
        ...
        return redisService.getData(this.getKeys(data)).onItem().transformToUni(fetchedData -> Uni.createFrom().voidItem().invoke(() -> {
            for (var map : fetchedData) {
                if (!map.isEmpty()){
                    var obj = objectMapper.convertValue(map, CusEmergencyURGPriceListItem.class);
                    data.getCusEmergencyUrgPriceListItems().add(obj);
                }
            }
        }));
