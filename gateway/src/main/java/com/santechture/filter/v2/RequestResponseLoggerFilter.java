package com.santechture.filter.v2;
...
public class RequestResponseLoggerFilter implements ContainerRequestFilter, ContainerResponseFilter {
    ...
    public void filter(ContainerRequestContext requestContext) throws IOException {
        ...
        if (requestContext.getUriInfo().getRequestUri().getPath().startsWith("/api/v2/price/qatar")) {
            ...
            var market = PriceMarket.QATAR;
            var type = TypeEnum.QTR;
...
//                        logger.info(responseBody);
                        var genericGeneralResponse = objectMapper.readValue(responseString, GenericGeneralResponse.class);
                        if (genericGeneralResponse.getStatusCode() != 404 ){
                            claim = objectMapper.convertValue(genericGeneralResponse.getData(), Claim.class);
