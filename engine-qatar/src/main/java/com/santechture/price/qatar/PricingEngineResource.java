package com.santechture.price.qatar;
...
//    @WithSpan("Price Engine")

    public Data getPrice(Data data) {

        logger.info("new request");
        var session = initializeSession(data);
    ...
    @WithSpan("Initialize session")
    public KieSession initializeSession(Data data) {
        ...
        if (data.getClaim().getPatient() != null && data.getClaim().getPatient().getPatientInsurance() != null) {
            ...
            if (data.getClaim().getPatient().getPatientInsurance().getCoinsurance() != null) {
                sessionService.addListToSession(data.getClaim().getPatient().getPatientInsurance().getCoinsurance(), session);
            }
        }
        ...
        sessionService.addListToSession(data.getCusPriceList(), session);
        ...
        sessionService.addListToSession(data.getCusIP_ARDRGPriceListItems(), session);
        sessionService.addListToSession(data.getCusDentistryAsdsgPriceListItems(), session);
        sessionService.addListToSession(data.getCusEmergencyUrgPriceListItems(), session);
        sessionService.addListToSession(data.getCusRadiologyAchiPriceListItems(), session);
        sessionService.addListToSession(data.getCusOpQocsPriceListItems(), session);
        sessionService.addListToSession(data.getCusIPSubPriceListItems(), session);
        sessionService.addListToSession(data.getMedicinesPriceListItems(), session);
        return session;
    }

    @WithSpan("Fire all rules")
